...
To find out why it wasn't working, I had to read quite a bit about how USB works. Below are the parts that are relevant for the problem or were confusing to me.

### Data

The protocol itself has change quite a bit from when it first launched in 1996. From the original release of 1.0 through the 2.0 release, the connections were half-duplex. Since USB 3.0, the actual protocol fundamentally changed, where it is now a full-duplex connection just like all Ethernet standards since gigabit ethernet.

Next, USB 3.1 was introduced. The new standard didn't exist side-by-side with USB 3.0, but superseded the 3.0 standard. What was 3.0 then became know as 3.1 Gen 1. The new standard also introduced a new variant which supports up to 10 Gbit/s.
...
Initially, the USB standard supplied only enough power to run the attached devices. However as time went on, the USB standard was used to provide power to more and more devices, so official standards were created for supplying more power. This culminated in 2012 into what is now the USB Power Delivery standard. 
